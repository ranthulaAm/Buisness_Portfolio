...
import { listenToBrandCollaborations, BrandCollaboration } from '../services/brandCollabService';
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ user }) => {
  ...
  const [brandCollabs, setBrandCollabs] = useState<BrandCollaboration[]>([]);
  ...
  useEffect(() => {
    ...
    const unsubscribeCollabs = listenToBrandCollaborations((data) => {
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex flex-col xl:flex-row justify-between xl:items-end mb-4 gap-4">
          ...
          <div className="space-y-6">
            ...
            {/* 🤝 Brand Collaboration Revenue Analytics - Separated Section */}
            <div className="border-t border-gray-100 dark:border-zinc-800 pt-8 mt-8 space-y-6" id="brand-collab-analytics">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  <h3 className="text-xl font-black text-gray-800 dark:text-white flex items-center gap-2 font-display">
                    🤝 Brand Collaboration Financials
                  </h3>
                  <p className="text-xs text-gray-400">Independent analysis of contract revenues, payments, and brand client health.</p>
                </div>
                <div className="bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/30 px-3.5 py-1.5 rounded-xl text-xs text-emerald-700 dark:text-emerald-400 font-semibold flex items-center gap-1.5 self-start md:self-auto">
                  <span className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></span>
                  Active Tracker
                </div>
              </div>

              {/* Brand Collab Stat Cards */}
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                <div className="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-2xl shadow-sm p-5 flex flex-col justify-between hover:border-blue-500/50 transition-colors">
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2 block font-display">Total Contract Value</span>
                  <div>
                    <div className="text-xl font-black text-gray-800 dark:text-white mb-1">
                      LKR {brandCollabs.reduce((sum, c) => sum + (c.totalPrice || 0), 0).toLocaleString()}
                    </div>
                    <span className="text-[10px] text-gray-400">Across {brandCollabs.length} campaigns</span>
                  </div>
                </div>

                <div className="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-2xl shadow-sm p-5 flex flex-col justify-between hover:border-emerald-500/50 transition-colors">
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2 block font-display">Total Brand Capital Collected</span>
                  <div>
                    <div className="text-xl font-black text-emerald-600 dark:text-emerald-400 mb-1">
                      LKR {brandCollabs.reduce((sum, c) => sum + (c.paymentHistory || []).reduce((pSum, p) => pSum + p.amount, 0), 0).toLocaleString()}
                    </div>
                    <span className="text-[10px] text-emerald-500">Realized revenue</span>
                  </div>
                </div>

                <div className="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-2xl shadow-sm p-5 flex flex-col justify-between hover:border-red-500/50 transition-colors">
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2 block font-display">Total Outstanding Balance</span>
                  <div>
                    <div className="text-xl font-black text-red-500 mb-1">
                      LKR {(
                        brandCollabs.reduce((sum, c) => sum + (c.totalPrice || 0), 0) -
                        brandCollabs.reduce((sum, c) => sum + (c.paymentHistory || []).reduce((pSum, p) => pSum + p.amount, 0), 0)
                      ).toLocaleString()}
                    </div>
                    <span className="text-[10px] text-red-400">Uncollected payments</span>
                  </div>
                </div>

                <div className="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-2xl shadow-sm p-5 flex flex-col justify-between hover:border-indigo-500/50 transition-colors">
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2 block font-display">Campaign Breakdown</span>
                  <div>
                    <div className="text-xl font-black text-gray-800 dark:text-white mb-1">
                      {brandCollabs.filter(c => c.status === 'active').length} Active
                    </div>
                    <span className="text-[10px] text-indigo-500">
                      {brandCollabs.filter(c => c.status === 'completed').length} completed, {brandCollabs.filter(c => c.status === 'archived').length} archived
                    </span>
                  </div>
                </div>
              </div>

              {/* Brand Collab Revenue Chart vs. Collected Chart */}
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-2xl shadow-sm p-6 lg:col-span-2">
                  <h4 className="text-sm font-bold text-gray-800 dark:text-white mb-4 font-display">Brand Performance: Contracts vs. Receipts</h4>
                  <div className="h-[280px] w-full min-h-[280px]">
                    {brandCollabs.length === 0 ? (
                      <div className="h-full flex items-center justify-center text-gray-400 text-xs italic">No brand collaboration data yet</div>
                    ) : (
                      <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0} debounce={50}>
                        <BarChart
                          data={brandCollabs.map(c => {
                            const totalPaid = (c.paymentHistory || []).reduce((sum, p) => sum + p.amount, 0);
                            return {
                              name: c.brandName.length > 15 ? c.brandName.substring(0, 15) + '...' : c.brandName,
                              'Contract Price (LKR)': c.totalPrice,
                              'Collected (LKR)': totalPaid,
                            };
                          })}
                        >
                          ...
                          <RechartsTooltip cursor={{ fill: 'rgba(229, 231, 235, 0.2)' }} contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }} />
                          <Legend wrapperStyle={{ fontSize: 11 }} />
                          <Bar dataKey="Contract Price (LKR)" fill="#3b82f6" radius={[4, 4, 0, 0]} />
                          <Bar dataKey="Collected (LKR)" fill="#10b981" radius={[4, 4, 0, 0]} />
                        </BarChart>
                      </ResponsiveContainer>
                    )}
                  </div>
                ...
                <div className="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-2xl shadow-sm p-6 flex flex-col justify-between">
                  <div>
                    <h4 className="text-sm font-bold text-gray-800 dark:text-white mb-3 font-display">Revenue Realization Rate</h4>
                    <p className="text-xs text-gray-400 mb-6">Percentage of total contracted brand revenue that has been fully collected to date.</p>
                  </div>
                  {(() => {
                    const totalVal = brandCollabs.reduce((sum, c) => sum + (c.totalPrice || 0), 0);
                    const totalCollected = brandCollabs.reduce((sum, c) => sum + (c.paymentHistory || []).reduce((pSum, p) => pSum + p.amount, 0), 0);
                    const pct = totalVal > 0 ? Math.round((totalCollected / totalVal) * 100) : 0;
                      ...
                      <div className="flex flex-col items-center justify-center py-4">
                        <div className="relative flex items-center justify-center w-36 h-36">
                          ...
                          <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                            <circle cx="50" cy="50" r="40" stroke="rgba(229, 231, 235, 0.4)" strokeWidth="8" fill="transparent" />
                            <circle
                              cx="50"
                              cy="50"
                              r="40"
                          ...
                          <div className="absolute text-center">
                            <span className="text-2xl font-black text-gray-800 dark:text-white font-display">{pct}%</span>
                            <span className="block text-[8px] uppercase tracking-wider text-gray-400 font-bold mt-0.5">Realized</span>
                          </div>
                        </div>
                        <div className="text-center mt-6">
                          <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 block">Outstanding Collection</span>
                          <span className="text-sm font-black text-gray-700 dark:text-white">LKR {(totalVal - totalCollected).toLocaleString()}</span>
                        </div>
                      </div>
                    );
                  })()}
