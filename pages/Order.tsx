...
const PhotoshopColorPicker: React.FC<{ color: string; onChange: (hex: string) => void; onAdd: (hex: string) => void; isPrintMode: boolean; canAdd: boolean; }> = ({ color, onChange, onAdd, isPrintMode, canAdd }) => {
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 p-5 rounded-[2.5rem] shadow-sm w-full max-w-sm animate-fade-in h-fit">
      <div className="flex gap-4 h-48 mb-5">
        <div className="relative flex-1">
          <canvas ref={canvasRef} width={220} height={220} className="w-full h-full rounded-2xl cursor-crosshair border border-gray-200 dark:border-zinc-700 touch-none" onMouseDown={(e) => { setIsDraggingSV(true); updateSV(e.clientX, e.clientY); }} onTouchStart={(e) => { setIsDraggingSV(true); updateSV(e.touches[0].clientX, e.touches[0].clientY); }} />
          <div className="absolute w-4 h-4 border-2 border-white dark:border-zinc-700 rounded-full shadow-lg pointer-events-none -translate-x-1/2 -translate-y-1/2" style={{ left: `${s}%`, top: `${100 - v}%`, backgroundColor: color }} />
        </div>
        <div className="w-8 h-full relative rounded-xl overflow-hidden border border-gray-200 dark:border-zinc-700 bg-gray-50 dark:bg-zinc-800">
          ...
          <div className="absolute w-full h-2 border-y border-white dark:border-zinc-700 shadow-md pointer-events-none" style={{ top: `${(h / 360) * 100}%` }} />
        </div>
      </div>
      <div className="bg-slate-50 dark:bg-zinc-900 p-4 rounded-2xl border border-gray-150 mb-5">
        <div className="flex items-center justify-between gap-6 mb-4">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl border-2 border-gray-200 dark:border-zinc-700 shadow-inner" style={{ backgroundColor: color }}></div>
            <div className="text-left">
              ...
              <div className="text-gray-900 dark:text-white font-mono text-sm font-bold uppercase tracking-wider">{color}</div>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 text-[10px] font-mono text-gray-500 dark:text-gray-400 border-l border-gray-200 dark:border-zinc-700 pl-5">
...
export const Order: React.FC<OrderProps> = ({ user, onLoginRequest }) => {
  ...
  const renderDynamicForm = () => {
    ...
    const inputClass = (errKey: string) => `w-full bg-slate-50 border rounded-2xl px-6 py-4 text-gray-900 dark:text-white placeholder:text-gray-400 outline-none transition-all focus:bg-white dark:bg-zinc-900 ${errors[errKey] ? 'border-red-500 bg-red-50/20 shadow-sm' : 'border-zinc-300 focus:border-purple-600 focus:ring-4 focus:ring-purple-100 shadow-sm'}`;
      ...
      return (
        <div className="space-y-6">
          {selectedService.customFields.map(field => {
              ...
              return (
                <div key={field.id}>
                  ...
                  <select
                    ...
                    className={inputClass(field.id)}
                  >
                    <option value="" className="bg-white dark:bg-zinc-900 text-gray-400">
                      {field.placeholder || "Select an option..."}
                    </option>
                    {field.options?.map((opt, idx) => (
                      <option key={idx} value={opt} className="bg-white dark:bg-zinc-900 text-gray-900 dark:text-white">
                        {opt}
                      </option>
                    ))}
                  </select>
            ...
            if (field.type === 'checkbox') {
              return (
                <div key={field.id} className="flex items-start gap-3 p-4 bg-slate-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-700 rounded-2xl">
                  <input
                    type="checkbox"
                    id={`client_check_${field.id}`}
                    checked={!!formData.customFields[field.id]}
                    onChange={e => handleCustomFieldChange(field.id, e.target.checked)}
                    className="mt-1 rounded border-gray-300 dark:border-zinc-600 text-purple-600 focus:ring-purple-500 w-4 h-4 cursor-pointer"
                  />
                  <div>
                    <label htmlFor={`client_check_${field.id}`} className="text-sm font-semibold text-gray-900 dark:text-white cursor-pointer select-none">
                      {field.label} {field.required && <span className="text-red-500">*</span>}
                    </label>
                    {field.placeholder && (
          ...
          <div><input name="brandName" value={formData.brandName} onChange={handleInputChange} placeholder="Brand or Company Name *" className={inputClass('brandName')} /><ErrorMsg name="brandName" /></div>
           <div className="space-y-3">
            ...
            {formData.socialLinks.map((link, i) => (
              <div key={i} className="flex gap-3"><select value={link.platform} onChange={(e) => updateList('socialLinks', i, 'platform', e.target.value)} className="bg-gray-50 dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-xl px-4 py-2 text-gray-900 dark:text-white text-xs outline-none focus:border-purple-600 focus:bg-white dark:bg-zinc-900"><option className="bg-white dark:bg-zinc-900">WhatsApp</option><option className="bg-white dark:bg-zinc-900">Facebook</option><option className="bg-white dark:bg-zinc-900">Instagram</option><option className="bg-white dark:bg-zinc-900">TikTok</option><option className="bg-white dark:bg-zinc-900">LinkedIn</option></select><input value={link.handle} onChange={(e) => updateList('socialLinks', i, 'handle', e.target.value)} placeholder="@handle" className="flex-1 bg-gray-50 dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-xl px-4 py-2 text-gray-900 dark:text-white text-xs outline-none focus:border-purple-600 focus:bg-white dark:bg-zinc-900" />{formData.socialLinks.length > 1 && <button type="button" onClick={() => removeItem('socialLinks', i)} className="text-gray-400 hover:text-red-500"><MinusCircle size={20} /></button>}</div>
            ))}
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="mb-10 text-center"><h1 className="text-5xl md:text-7xl font-display text-gray-900 dark:text-white mb-4 uppercase tracking-tighter leading-none">{editOrderId ? 'Edit Project' : selectedServiceTitle}</h1><p className="text-gray-500 dark:text-gray-400 font-light text-lg">{editOrderId ? 'Update your requirements' : 'Just 4 simple steps to bring your idea to life.'}</p></div>

      <div ref={formRef} className="bg-white dark:bg-zinc-900 rounded-[2rem] shadow-[0_15px_60px_rgba(0,0,0,0.07)] relative overflow-hidden flex flex-col md:flex-row min-h-[650px] border border-zinc-300">
        <div className="hidden md:flex flex-col w-64 bg-gray-50/70 border-r border-gray-100 dark:border-zinc-700 p-10">
          {[1, 2, 3, 4].map(s => (
            <div key={s} className={`flex items-center gap-4 mb-10 transition-all ${step === s ? 'text-purple-600 font-bold' : step > s ? 'text-gray-900 dark:text-white' : 'text-gray-300'}`}>
              <div className={`w-10 h-10 rounded-full border-2 flex items-center justify-center font-black text-sm transition-all ${step === s ? 'border-purple-600 text-purple-600 bg-purple-50' : step > s ? 'border-purple-600 bg-purple-600 text-white shadow-[0_4px_12px_rgba(147,51,234,0.15)]' : 'border-gray-200 dark:border-zinc-700 text-gray-300'}`}>{step > s ? <Check size={18} strokeWidth={3} /> : s}</div>
        ...
        <div ref={scrollContainerRef} className="flex-1 p-8 md:p-14 relative bg-transparent overflow-y-auto">
          <form onSubmit={handleSubmit} className="h-full flex flex-col">
            ...
            <div className="flex md:hidden items-center justify-between gap-1 mb-8 pb-4 border-b border-gray-150 dark:border-zinc-700">
              {[1, 2, 3, 4].map(s => (
                <div key={s} className="flex-grow flex flex-col items-center">
                  <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center font-black text-xs transition-all ${step === s ? 'border-purple-600 text-purple-600 bg-purple-50 dark:bg-purple-950/20' : step > s ? 'border-purple-600 bg-purple-600 text-white' : 'border-gray-200 dark:border-zinc-700 text-gray-300 dark:text-slate-600'}`}>
                    {step > s ? <Check size={14} strokeWidth={3} /> : s}
                  </div>
                  <span className={`text-[8px] font-black tracking-wider uppercase mt-1.5 ${step === s ? 'text-purple-600 dark:text-purple-400 font-extrabold' : 'text-gray-400 dark:text-gray-500'}`}>
            ...
            <div className={getStepClasses(1)}>
              <h3 className="text-3xl font-display text-gray-900 dark:text-white mb-8 uppercase tracking-tight border-b border-gray-100 dark:border-zinc-700 pb-4">Basic Info</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="md:col-span-2"><label className="block text-[10px] font-black text-purple-600 mb-3 uppercase tracking-widest font-extrabold">Service</label><select name="serviceType" value={formData.serviceType} onChange={handleInputChange} disabled={!!editOrderId} className={`w-full bg-slate-50 border border-zinc-300 rounded-2xl px-6 py-5 text-gray-950 font-bold cursor-pointer outline-none focus:border-purple-600 focus:bg-white dark:bg-zinc-900 transition-all shadow-sm ${editOrderId ? 'opacity-50 cursor-not-allowed' : ''}`}>{services.map(s => <option key={s.id} value={s.id} className="bg-white dark:bg-zinc-900 text-gray-900 dark:text-white">{s.title} — LKR {s.price}</option>)}</select></div>
                <div><input name="name" value={formData.name} onChange={handleInputChange} placeholder="Full Name *" className={`w-full bg-slate-50 border rounded-2xl px-6 py-4 text-gray-900 dark:text-white placeholder:text-gray-400 outline-none focus:border-purple-600 focus:bg-white dark:bg-zinc-900 transition-all shadow-sm ${errors.name ? 'border-red-500 bg-red-50/20' : 'border-zinc-300'}`} />{errors.name && <div className="text-red-500 text-[10px] font-bold mt-1 flex items-center gap-1"><AlertCircle size={10} /> {errors.name}</div>}</div>
                <div>
                    ...
                    <div className="flex gap-2">
                        <select 
                            ...
                            className="bg-slate-50 border border-zinc-300 rounded-2xl px-4 py-4 text-gray-900 dark:text-white outline-none focus:border-purple-600 focus:bg-white dark:bg-zinc-900 appearance-none text-center min-w-[80px] shadow-sm font-bold"
                        >
                            <option value="+94" className="bg-white dark:bg-zinc-900 text-gray-900 dark:text-white">🇱🇰 +94</option>
                            <option value="+1" className="bg-white dark:bg-zinc-900 text-gray-900 dark:text-white">🇺🇸 +1</option>
                            <option value="+44" className="bg-white dark:bg-zinc-900 text-gray-900 dark:text-white">🇬🇧 +44</option>
                            <option value="+61" className="bg-white dark:bg-zinc-900 text-gray-900 dark:text-white">🇦🇺 +61</option>
                            <option value="+91" className="bg-white dark:bg-zinc-900 text-gray-900 dark:text-white">🇮🇳 +91</option>
                            <option value="+971" className="bg-white dark:bg-zinc-900 text-gray-900 dark:text-white">🇦🇪 +971</option>
                            <option value="" className="bg-white dark:bg-zinc-900 text-gray-900 dark:text-white">Other</option>
                        </select>
                        <input 
                            type="tel" 
                            value={phoneInput} 
                            ...
                            className={`flex-1 bg-slate-50 border rounded-2xl px-6 py-4 text-gray-900 dark:text-white placeholder:text-gray-400 ${errors.mobile ? 'border-red-500 bg-red-50/20' : 'border-zinc-300'} outline-none focus:border-purple-600 focus:bg-white dark:bg-zinc-900 transition-all shadow-sm font-semibold`} 
                        />
                    </div>
                ...
                <div className="md:col-span-2"><input name="email" value={formData.email} onChange={handleInputChange} placeholder="Email Address *" className={`w-full bg-slate-50 border rounded-2xl px-6 py-4 text-gray-900 dark:text-white placeholder:text-gray-400 outline-none focus:border-purple-600 focus:bg-white dark:bg-zinc-900 transition-all shadow-sm ${errors.email ? 'border-red-500 bg-red-50/20' : 'border-zinc-300'}`} />{errors.email && <div className="text-red-500 text-[10px] font-bold mt-1 flex items-center gap-1"><AlertCircle size={10} /> {errors.email}</div>}</div>
              </div>
            </div>

            {/* STEP 2: PROJECT DETAILS */}
            <div className={getStepClasses(2)}>
              <h3 className="text-3xl font-display text-gray-900 dark:text-white mb-8 uppercase tracking-tight border-b border-gray-100 dark:border-zinc-700 pb-4">Project Details</h3>
              <div className="space-y-10">
                {renderDynamicForm()}
                
                <div className="pt-8 border-t border-gray-100 dark:border-zinc-700">
                  ...
                  <textarea 
                    ...
                    className="w-full bg-slate-50 border border-zinc-300 rounded-2xl px-6 py-4 text-gray-900 dark:text-white placeholder:text-gray-400 outline-none focus:border-purple-600 focus:bg-white dark:bg-zinc-900 transition-all shadow-sm"
                  ></textarea>
                </div>

                <div className="pt-8 border-t border-gray-100 dark:border-zinc-700"><label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4 block">Can't describe it in text? Use voice:</label><div className="flex flex-wrap items-center gap-4"><button type="button" onClick={isRecording ? stopRecording : startRecording} className={`flex items-center gap-3 px-6 py-3 rounded-full font-bold text-xs uppercase tracking-widest transition-all ${isRecording ? 'bg-pink-500 text-white animate-pulse' : 'bg-gray-100 dark:bg-zinc-800 text-gray-700 dark:text-gray-300 hover:bg-gray-250 border border-gray-200 dark:border-zinc-700 hover:bg-gray-200'}`}>{isRecording ? <><Square size={14} fill="white" /> Stop Recording</> : <><Mic size={14} /> Record Voice</>}</button>{isRecording && <span className="text-pink-500 text-xs font-bold animate-pulse">Recording...</span>}{isConverting && <span className="text-purple-500 text-xs font-bold animate-pulse">Converting to MP3...</span>}</div></div>
                {formData.voiceClips.length > 0 && <div className="space-y-3"><label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Voice Notes</label>{formData.voiceClips.map((clip, i) => (<div key={i} className="flex flex-col sm:flex-row sm:items-center justify-between bg-gray-50 dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 p-4 rounded-2xl relative overflow-hidden gap-3">
                    ...
                    <div className="flex items-center gap-3 z-10"><Play size={14} className="text-purple-600 shrink-0" /><span className="text-sm font-medium text-gray-700 dark:text-gray-300 truncate max-w-[150px] sm:max-w-xs">{clip.name}</span>
                    {uploadProgress[clip.name] !== undefined && (
            ...
            <div className={getStepClasses(3)}>
                ...
                <div className="space-y-8">
                  <div className="flex items-center justify-between border-b border-gray-100 dark:border-zinc-700 pb-4">
                    <h3 className="text-3xl font-display text-gray-900 dark:text-white uppercase tracking-tight">Dimension & Layout</h3>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                      {currentPresets.map(p => {
                        ...
                        return (
                          <button key={p.id} type="button" onClick={() => applyPreset(p.id)} className={`p-4 rounded-2xl text-left transition-all border group relative overflow-hidden ${isSelected ? 'bg-purple-50 dark:bg-purple-900/30 border-purple-600 dark:border-purple-500' : 'bg-gray-50/50 border-gray-200 dark:border-zinc-700 hover:border-gray-300 dark:border-zinc-600'}`}>
                            <div className="flex items-center justify-between mb-3"><Icon size={18} className={isSelected ? 'text-purple-600' : 'text-gray-400 group-hover:text-gray-600 dark:text-gray-400'} />{isSelected && <Check size={14} className="text-purple-600" />}</div>
                            <div className={`text-[11px] font-black uppercase tracking-wider mb-1 ${isSelected ? 'text-gray-950' : 'text-gray-650 group-hover:text-gray-800 dark:text-gray-200'}`}>{p.name}</div>
                            <div className="text-[10px] font-mono text-gray-400 group-hover:text-gray-500 dark:text-gray-400">{p.width}x{p.height}{p.unit} @ {p.ppi}ppi</div>
                          </button>
                        );
                      })}
                      {currentPresets.length === 0 && <div className="col-span-full py-8 text-center bg-gray-5 border border-dashed border-gray-200 dark:border-zinc-700 rounded-2xl"><p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest italic">Manual setup required for this mode</p></div>}
                    </div>
                  </div>

                  <div className="bg-slate-50 dark:bg-zinc-900/50 rounded-3xl p-8 border border-zinc-300 dark:border-zinc-700 space-y-8 shadow-sm">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                      <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Width</label><input type="number" value={formData.dimensions.width} onChange={(e) => handleDimensionChange('width', e.target.value)} className="w-full bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl px-5 py-4 text-gray-900 dark:text-white font-mono text-sm focus:border-purple-600 outline-none transition-all h-12" /></div>
                      <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Height</label><input type="number" value={formData.dimensions.height} onChange={(e) => handleDimensionChange('height', e.target.value)} className="w-full bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl px-5 py-4 text-gray-900 dark:text-white font-mono text-sm focus:border-purple-600 outline-none transition-all h-12" /></div>
                      <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Unit</label><select value={formData.dimensions.unit} onChange={(e) => handleDimensionChange('unit', e.target.value)} className="w-full bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl px-4 py-3 text-gray-900 dark:text-white text-xs outline-none focus:border-purple-600 cursor-pointer h-12"><option value="px">Pixels (px)</option><option value="in">Inches (in)</option><option value="mm">Millimeters (mm)</option><option value="cm">Centimeters (cm)</option><option value="m">Meters (m)</option><option value="pt">Points (pt)</option><option value="pc">Picas (pc)</option><option value="ft">Feet (ft)</option><option value="yd">Yards (yd)</option></select></div>
                    </div>

                    <div className="flex flex-wrap items-center justify-between gap-8 pt-8 border-t border-gray-100 dark:border-zinc-700">
                      <div className="flex flex-wrap items-center gap-6 sm:gap-10">
                        <div className="space-y-3"><label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Orientation</label><div className="flex bg-white dark:bg-zinc-900 rounded-xl p-1 text-gray-300 border border-gray-200 dark:border-zinc-700"><button type="button" onClick={toggleOrientation} className={`p-3 rounded-lg transition-all flex items-center justify-center gap-2 ${formData.dimensions.orientation === 'portrait' ? 'bg-purple-600 text-white shadow-sm' : 'text-gray-400 hover:text-gray-700 dark:text-gray-300'}`}><div className="w-3 h-4 border-2 border-current rounded-sm"></div></button><button type="button" onClick={toggleOrientation} className={`p-3 rounded-lg transition-all flex items-center justify-center gap-2 ${formData.dimensions.orientation === 'landscape' ? 'bg-purple-600 text-white shadow-sm' : 'text-gray-400 hover:text-gray-700 dark:text-gray-300'}`}><div className="w-4 h-3 border-2 border-current rounded-sm"></div></button></div></div>
                        <div className="space-y-3"><label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Resolution (PPI)</label><select value={formData.dimensions.ppi} onChange={(e) => handleDimensionChange('ppi', e.target.value)} className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl px-5 py-3 text-gray-900 dark:text-white text-xs outline-none block focus:border-purple-600 cursor-pointer h-12 align-middle"><option value="72">72 (Screen / Web)</option><option value="150">150 (Digital Print)</option><option value="300">300 (Offset Print)</option></select></div>
                      </div>
                  ...
                  <div><label className="text-[10px] font-black text-gray-400 mb-3 block uppercase tracking-widest">Style Keywords</label><input name="keywords" value={formData.keywords} onChange={handleInputChange} placeholder="Modern, Bold, Minimalist, Luxury..." className="w-full bg-slate-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-2xl px-6 py-4 text-gray-900 dark:text-white placeholder:text-gray-400 outline-none focus:border-purple-600 focus:bg-white dark:bg-zinc-900 transition-all shadow-sm" /></div>
                </div>

                <div className="space-y-8 font-sans">
                  <h3 className="text-3xl font-display text-gray-900 dark:text-white mb-6 uppercase tracking-tight border-b border-gray-100 dark:border-zinc-700 pb-4">Style & Colors</h3>
                  <div className="flex flex-col items-center xl:items-start gap-10">
                    ...
                    <div className="w-full max-w-sm"><label className="text-[10px] font-black text-gray-400 mb-5 block uppercase tracking-widest flex items-center justify-between">Chosen Palette <span className="text-[9px] font-mono text-gray-400">{formData.colorPalette.length}/5 Colors</span></label><div className="flex flex-wrap gap-4">{formData.colorPalette.length === 0 && <div className="w-full py-4 text-center border border-dashed border-gray-200 dark:border-zinc-700 rounded-2xl"><span className="text-[10px] text-gray-400 uppercase font-black italic tracking-widest">Add colors from picker above</span></div>}{formData.colorPalette.map(c => (<div key={c} className="flex items-center gap-3 bg-white dark:bg-zinc-900 rounded-full px-5 py-2.5 border border-gray-200 dark:border-zinc-700 group animate-fade-in hover:border-gray-300 dark:border-zinc-600 transition-all shadow-md"><div className="w-4 h-4 rounded-full shadow-sm" style={{ backgroundColor: c }}></div><span className="text-[10px] font-mono font-black uppercase text-gray-700 dark:text-gray-300">{c}</span><button type="button" onClick={() => setFormData(prev => ({ ...prev, colorPalette: prev.colorPalette.filter(x => x !== c) }))} className="text-gray-400 hover:text-red-500 transition-colors ml-2"><X size={14} /></button></div>))}</div></div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className={getStepClasses(4)}>
              <h3 className="text-3xl font-display text-gray-900 dark:text-white mb-8 uppercase tracking-tight border-b border-gray-100 dark:border-zinc-700 pb-4">Upload Assets</h3>
              <div className="space-y-8">
                <div className="border-2 border-dashed border-gray-200 dark:border-zinc-700 rounded-[2.5rem] p-12 text-center bg-gray-50/50 hover:bg-gray-100 dark:hover:bg-slate-700/30 hover:border-gray-300 dark:border-zinc-600 transition-all relative group cursor-pointer"><input type="file" multiple className="absolute inset-0 opacity-0 cursor-pointer z-20" onChange={(e) => e.target.files?.length && setFormData(p => ({ ...p, files: [...p.files, ...Array.from(e.target.files!)] }))} /><Upload className="mx-auto mb-4 text-purple-600 animate-pulse" size={40} /><p className="text-lg font-bold uppercase tracking-widest text-gray-600 dark:text-gray-400 group-hover:text-gray-900 dark:text-white">Choose Photos or Logos</p></div>
                {formData.files.length > 0 && <div className="flex flex-col gap-3">{formData.files.map((f, i) => (
                    <div key={i} className="text-sm font-medium bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 px-5 py-3 rounded-2xl text-gray-700 dark:text-gray-300 flex items-center gap-4 group hover:border-red-200 transition-colors shadow-sm relative overflow-hidden">
                ...
                <div className="bg-gray-50 dark:bg-zinc-800 p-8 rounded-[2rem] border border-gray-200 dark:border-zinc-700">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-xs pb-6 border-b border-gray-200 dark:border-zinc-700">
                    <div>
                      <p className="text-gray-400 uppercase text-[9px] mb-2 font-black">Project Type</p>
                      <p className="text-gray-800 dark:text-gray-200 font-bold text-sm">{selectedServiceTitle}</p>
                    </div>
                    <div>
                      <p className="text-gray-400 uppercase text-[9px] mb-2 font-black">Color Mode</p>
                      <p className="text-gray-800 dark:text-gray-200 font-bold text-sm">{isPrintMode ? 'Print Ready (CMYK)' : 'Web (RGB)'}</p>
                    </div>
                  </div>

                  {/* Promo Code Input & Application */}
                  <div className="py-6 border-b border-gray-200 dark:border-zinc-700">
                    ...
                    <div className="flex gap-2 w-full max-w-full sm:max-w-sm">
                      <input 
                        ...
                        className="flex-1 bg-white dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-700 rounded-xl px-4 py-2.5 text-xs font-mono font-bold uppercase tracking-wider outline-none focus:border-purple-600"
                      />
                      <button 
                        type="button" 
                  ...
                  <div className="pt-6 space-y-3">
                    ...
                    <div className="flex justify-between items-center text-sm font-bold text-gray-900 dark:text-white pt-3 border-t border-gray-200 dark:border-zinc-700">
            ...
            <div className="mt-auto flex justify-end items-center gap-4 pt-10 border-t border-gray-100 dark:border-zinc-700 sticky bottom-0 bg-gray-50/95 dark:bg-zinc-800/95 backdrop-blur-md pb-4">
               {step > 1 && (
                 <button 
                   type="button" 
                   onClick={() => changeStep(step - 1)} 
                   className="inline-flex items-center gap-1.5 text-purple-600 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300 active:scale-[0.96] bg-gray-100 dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 px-5 py-3 rounded-full shadow-sm hover:shadow font-bold text-xs uppercase tracking-widest transition-all duration-300 ease-in-out"
                 >
