  ...
  return (
    <footer className={`bg-gray-50 dark:bg-zinc-800 border-t border-gray-200 dark:border-zinc-700 pt-16 ${paddingBottomClass} relative z-20`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row justify-between items-start gap-12">
          <div className="text-center md:text-left">
            <div className="flex items-center justify-center md:justify-start gap-3 mb-4">
              ...
              <h3 className="text-2xl font-display font-bold text-gray-900 dark:text-white">I'm <span className="text-purple-600">Ranthula</span></h3>
            </div>
            <p className="text-gray-500 dark:text-gray-400 max-w-xs font-light mb-6">
          ...
          <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-8 text-sm max-w-2xl w-full md:mx-auto">
             <div className="flex flex-col gap-4">
                <h4 className="font-bold text-gray-900 dark:text-white uppercase tracking-widest text-xs mb-2">Contact</h4>
                <a href={`mailto:${footerData.email}`} className="flex items-center gap-3 text-gray-600 dark:text-gray-400 hover:text-purple-600 transition-colors">
                    <Mail size={16} className="text-gray-400" />
                    {footerData.email}
                </a>
                <a href={`tel:${footerData.phone.replace(/[^0-9+]/g, '')}`} className="flex items-center gap-3 text-gray-600 dark:text-gray-400 hover:text-purple-600 transition-colors">
             ...
             <div className="flex flex-col gap-4">
                <h4 className="font-bold text-gray-900 dark:text-white uppercase tracking-widest text-xs mb-2">Location & Links</h4>
                <div className="flex items-start gap-3 text-gray-600 dark:text-gray-400 whitespace-pre-wrap">
                    <MapPin size={16} className="text-gray-400 mt-0.5 shrink-0" />
                    <p>{footerData.location}</p>
                </div>
                {footerData.extraUrls && footerData.extraUrls.map((urlItem, idx) => (
                  <a key={idx} href={urlItem.url} target="_blank" rel="noreferrer" className="flex items-center gap-3 text-gray-600 dark:text-gray-400 hover:text-purple-600 transition-colors">
        ...
        <div className="mt-16 pt-8 border-t border-gray-200 dark:border-zinc-700 text-center text-sm text-gray-500 dark:text-gray-400 flex flex-col items-center gap-2">
