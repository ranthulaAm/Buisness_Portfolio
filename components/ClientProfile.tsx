...
export const ClientProfile: React.FC<ClientProfileProps> = ({ user, onLogout }) => {
  ...
  return (
    <div className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-gray-300 dark:border-zinc-600 rounded-3xl p-6 md:p-8 shadow-[0_8px_32px_rgba(0,0,0,0.12)] w-full max-w-full sm:max-w-3xl mx-auto space-y-8 mt-4">
      <div className="flex items-center gap-4 border-b border-gray-200 dark:border-zinc-700 pb-6 mb-6">
        <UserIcon size={24} className="text-gray-900 dark:text-white" />
        <h2 className="text-2xl font-bold font-display text-gray-900 dark:text-white">Client Profile</h2>
      </div>

      <div className="flex flex-col md:flex-row gap-8 items-start">
        <div className="flex flex-col items-center gap-4">
          <div className="relative w-32 h-32 rounded-full overflow-hidden border-4 border-white dark:border-zinc-700 shadow-lg bg-gray-100 dark:bg-zinc-800 flex items-center justify-center">
            {uploading ? (
        ...
        <div className="flex-1 space-y-6 w-full">
          <div>
            <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2 uppercase tracking-widest text-xs">Display Name</label>
            <input 
              type="text" 
              value={name} 
              onChange={(e) => setName(e.target.value)}
              className="w-full border border-gray-300 dark:border-zinc-600 rounded-xl px-4 py-3 outline-none focus:border-purple-500 bg-white dark:bg-zinc-900 font-medium text-gray-900 dark:text-white transition-colors"
            />
          </div>

          <div>
            <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2 uppercase tracking-widest text-xs">Email Address</label>
            <input 
              type="text" 
              value={user.email} 
              disabled
              className="w-full border border-gray-200 dark:border-zinc-700 rounded-xl px-4 py-3 bg-gray-100 dark:bg-zinc-800 font-medium text-gray-500 dark:text-gray-400 cursor-not-allowed"
            />
          </div>
          <div>
            <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2 uppercase tracking-widest text-xs">Phone Numbers (comma separated)</label>
            <input 
              type="text" 
              value={mobiles} 
      ...
      <div className="border-t border-gray-200 dark:border-zinc-700 pt-8 mt-8">
        ...
        <label className="flex items-center justify-between cursor-pointer group bg-gray-50 dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 p-4 rounded-2xl hover:bg-white dark:bg-zinc-900 transition-colors hover:border-gray-300 dark:border-zinc-600">
           <div className="flex flex-col">
              <span className="font-bold text-gray-900 dark:text-white text-sm">Order Status Updates</span>
              <span className="text-xs text-gray-500 dark:text-gray-400 font-medium">Receive an email when your project status changes</span>
           </div>
      ...
      <div className="border-t border-gray-200 dark:border-zinc-700 pt-8 mt-8 space-y-4">
        ...
        {securityLogs.length === 0 ? (
          <div className="border border-dashed border-gray-200 dark:border-zinc-700 rounded-2xl p-8 text-center bg-gray-50/50 dark:bg-zinc-800/20">
            <ShieldCheck size={32} className="mx-auto text-gray-400 mb-2" />
            <p className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">No file uploads detected yet</p>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Upload an avatar or project file to view real-time security integrity scans.</p>
          </div>
        ) : (
          <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
            {securityLogs.map((log) => {
              ...
              return (
                <div 
                  key={log.id} 
                  className={`border rounded-2xl p-4 flex flex-col md:flex-row gap-4 justify-between items-start md:items-center bg-white dark:bg-zinc-900 transition-all ${
                    log.status === 'passed' 
                      ? 'border-gray-200 dark:border-zinc-700 hover:border-gray-300 dark:hover:border-slate-700' 
                      : log.status === 'warning' 
                ...
                >
                  <div className="flex gap-3 items-start max-w-full">
                    ...
                    <div className="space-y-1 min-w-0 max-w-full">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="font-bold text-gray-900 dark:text-white text-sm truncate max-w-[200px] sm:max-w-xs block" title={log.fileName}>
                          {log.fileName}
                        </span>
                        <span className="text-[10px] font-mono text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-zinc-800 border border-gray-100 dark:border-zinc-700 px-1.5 py-0.5 rounded">
                          {formattedSize}
                        </span>
                      </div>
                      <div className="text-[11px] text-gray-500 dark:text-gray-400 flex flex-wrap gap-x-3 gap-y-1">
                        <span>Type: <span className="font-mono text-gray-700 dark:text-gray-300">{log.fileType}</span></span>
                        <span>•</span>
                      ...
                      <div className="flex flex-wrap gap-3 pt-1.5">
                        <div className="flex items-center gap-1 text-[10px] font-medium text-gray-600 dark:text-gray-400">
                  ...
                  <div className="flex items-center gap-3 w-full md:w-auto justify-between md:justify-end shrink-0 pt-2 md:pt-0 border-t md:border-t-0 border-gray-100 dark:border-zinc-700">
                    ...
                    <a 
                      ...
                      className="p-1.5 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-full transition-colors text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-slate-100"
