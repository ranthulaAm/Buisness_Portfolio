        ...
        <div className="space-y-8">
            <div className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl shadow-sm p-6 mb-8">
                ...
                <div className="space-y-6">
                    <div>
                        <label className="block text-xs font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400 mb-2">Subject Line</label>
                        <input 
                            type="text" 
                            value={config.emailSubjectTemplate} 
                            onChange={e => setConfig({...config, emailSubjectTemplate: e.target.value})}
                            className="w-full border border-gray-300 dark:border-zinc-600 rounded-xl p-3 outline-none focus:border-blue-500 font-mono text-sm"
                        />
                    </div>
                    <div>
                        <label className="block text-xs font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400 mb-2">Email Body</label>
                        <textarea 
                            value={config.emailBodyTemplate} 
                            onChange={e => setConfig({...config, emailBodyTemplate: e.target.value})}
                            className="w-full border border-gray-300 dark:border-zinc-600 rounded-xl p-3 outline-none focus:border-blue-500 min-h-[300px] font-mono text-sm"
                        />
                    </div>
                </div>

                <div className="flex justify-end pt-6 mt-6 border-t border-gray-100 dark:border-zinc-700">
