...
export const AdminContacts: React.FC = () => {
    ...
    return (
        <div className="space-y-8 pb-12">
            ...
            <div className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl shadow-sm p-6 max-w-4xl mx-auto relative">
                <ConfirmationDialog 
            ...
            {loading ? (
                ...
                <div className="space-y-4">
                    {contacts.map((c, i) => (
                        <div key={c.id || i} className={`border p-5 rounded-xl flex gap-4 \${c.isRead ? 'bg-white dark:bg-zinc-900 border-gray-200 dark:border-zinc-700' : 'bg-blue-50/30 border-blue-200 shadow-sm'}`}>
                            <div className="flex-1">
                                <div className="flex items-center justify-between mb-2">
                                    <div className="flex items-center gap-3">
                                        <h4 className={`font-bold \${c.isRead ? 'text-gray-900 dark:text-white' : 'text-blue-900'}`}>{c.name}</h4>
                                        <a href={`mailto:\${c.email}`} className="text-xs font-mono text-gray-500 dark:text-gray-400 hover:text-blue-600 bg-gray-100 dark:bg-zinc-800 px-2 py-0.5 rounded transition-colors">{c.email}</a>
                                        {c.whatsapp && (
            ...
            {orderModalOpen && selectedContact && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <div className="bg-white dark:bg-zinc-900 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 relative">
                        <button onClick={() => setOrderModalOpen(false)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-900 dark:text-white">
                            <X size={24} />
                        </button>
                        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Create Order</h2>
                        
                        <div className="space-y-4">
                            <div>
                                <label className="block text-xs font-bold uppercase text-gray-500 dark:text-gray-400 mb-1">Client Name</label>
                                <input type="text" value={selectedContact.name} readOnly className="w-full border border-gray-200 dark:border-zinc-700 rounded-lg px-3 py-2 bg-gray-50 dark:bg-zinc-800" />
                            </div>
                            
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-bold uppercase text-gray-500 dark:text-gray-400 mb-1">Client Email</label>
                                    <input type="text" value={selectedContact.email || ''} readOnly className="w-full border border-gray-200 dark:border-zinc-700 rounded-lg px-3 py-2 bg-gray-50 dark:bg-zinc-800" />
                                </div>
                                <div>
                                    <label className="block text-xs font-bold uppercase text-gray-500 dark:text-gray-400 mb-1">WhatsApp</label>
                                    <input type="text" value={selectedContact.whatsapp || ''} readOnly className="w-full border border-gray-200 dark:border-zinc-700 rounded-lg px-3 py-2 bg-gray-50 dark:bg-zinc-800" />
                                </div>
                            </div>

                            <div>
                                <label className="block text-xs font-bold uppercase text-gray-500 dark:text-gray-400 mb-1">Service Type</label>
                                <select 
                                    value={serviceId} 
                                    onChange={(e) => handleServiceChange(e.target.value)}
                                    className="w-full border border-gray-200 dark:border-zinc-700 rounded-lg px-3 py-2 outline-none focus:border-blue-500"
                                >
                                    {SERVICES.map(s => (
                            ...
                            <div>
                                <label className="block text-xs font-bold uppercase text-gray-500 dark:text-gray-400 mb-1">Order Requirements</label>
                                <textarea 
                                    value={requirements} 
                                    onChange={(e) => setRequirements(e.target.value)}
                                    className="w-full border border-gray-200 dark:border-zinc-700 rounded-lg px-3 py-2 outline-none focus:border-blue-500 min-h-[100px]"
                                />
                            </div>

                            <div>
                                <label className="block text-xs font-bold uppercase text-gray-500 dark:text-gray-400 mb-1">Quoted Price (LKR)</label>
                                <input 
                                    type="number" 
                                    value={price} 
                            ...
                            <div>
                                <label className="block text-xs font-bold uppercase text-gray-500 dark:text-gray-400 mb-2">Project Files (Manual Upload)</label>
                                <div className="border border-dashed border-gray-300 dark:border-zinc-600 rounded-xl p-6 text-center bg-gray-50 dark:bg-zinc-800">
                                    <input 
                                        type="file" 
                                        multiple 
                                ...
                                {files.length > 0 && (
                                    <ul className="mt-2 space-y-1">
                                        {files.map((file, i) => (
                                            <li key={i} className="text-xs text-gray-600 dark:text-gray-400 flex items-center justify-between bg-white dark:bg-zinc-900 p-2 border border-gray-100 dark:border-zinc-700 rounded">
